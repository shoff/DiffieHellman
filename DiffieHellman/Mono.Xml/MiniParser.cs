namespace DiffieHellman.Mono.Xml
{
	...
	internal class MiniParser
	{
		...
		public class AttrListImpl : IMutableAttrList, IAttrList
		{
			...
			public int Length => this.names.Count;

			public bool IsEmpty => this.Length != 0;

			public string[] Names => this.names.ToArray(typeof(string)) as string[];

			public string[] Values => this.values.ToArray(typeof(string)) as string[];
			...
			{
				Remove(this.names.IndexOf(name));
			}
		...
		public class XMLError : Exception
		{
			...
			{
				return $"{this.descr} @ (line = {this.line}, col = {this.column})";
			}
		}
